...
java -DentityExpansionLimit=100000000  -Dspring.profiles.active=cluster -Dlogging.file.path=${logslocation} -Dspring.datasource.username=${DB_USER} -Dspring.datasource.password=${DB_PWD}  $proxy_settings -Xms4096m -Xmx4096m -jar $base/gwas-association-parent-trait-mapper.jar ${efoIds}
